...
public class EntityManager {
    ...
    /**
     * Returns all entities that have at least all the specified component types.
     *
     * @param types component classes to check
     * @return collection of entities containing all given components
     */
    @SafeVarargs
    public final <T extends Component> Set<Entity> getEntitiesWithComponentSet(Class<? extends Component>... types) {
        if (types == null || types.length == 0 || componentPools == null) {
            return Collections.emptySet();
        }

        // Start with entities from the first type
        ComponentPool<? extends Component> firstPool = componentPools.getPool(types[0]);
        if (firstPool == null) {
            return Collections.emptySet();
        }

        Set<Entity> result = new HashSet<>(firstPool.getEntities());
        if (result.isEmpty()) {
            return Collections.emptySet();
        }

        // Intersect with entities from each other pool
        for (int i = 1; i < types.length; i++) {
            ComponentPool<? extends Component> pool = componentPools.getPool(types[i]);
            if (pool == null) {
                return Collections.emptySet(); // if one pool doesn't exist, no entity has it
            }
            Set<Entity> entities = pool.getEntities();
            if (entities == null || entities.isEmpty()) {
                return Collections.emptySet();
            }
            result.retainAll(entities);
            if (result.isEmpty()) {
                return Collections.emptySet(); // early exit
            }
        }
